...
async function renderAtendimento(){
  ...
  var years=[...new Set(products.map(gfYear).filter(Boolean))].sort(function(a,b){return b-a}),year=Number(state.attendanceYear||years[0]||2027);
  state.attendanceItems=state.attendanceItems||new Set();state.attendanceStage=state.attendanceStage||"Contato";
  ...
  function drawCatalog(){
    ...
    $("#catalogArea").className="";$("#catalogArea").innerHTML=html+"</div>";$("#attTotal").textContent=money(products.filter(function(p){return state.attendanceItems.has(p.ID_PRODUTO)}).reduce(function(a,p){return a+Number(p.VALOR_BASE||0)},0));
    $$("[data-att-product]").forEach(function(x){x.onchange=function(){x.checked?state.attendanceItems.add(x.dataset.attProduct):state.attendanceItems.delete(x.dataset.attProduct);drawCatalog()}});
    ...
    $("#flyerShortcut").onclick=function(){state.flyerYear=y;state.flyerSeries=s;navigate("panfletos")};
  }
  $("#attSerie").onchange=drawCatalog;$("#attYear").onchange=drawCatalog;
  $$("#stageFlow [data-stage]").forEach(function(x){x.classList.toggle("active",x.dataset.stage===state.attendanceStage);x.onclick=function(){state.attendanceStage=x.dataset.stage;$$("#stageFlow [data-stage]").forEach(function(b){b.classList.toggle("active",b===x)});$("#stagePct").textContent=GF_PCT[state.attendanceStage]+"%";$("#stageBar").style.width=GF_PCT[state.attendanceStage]+"%"}});
  $("#attStudent").onchange=function(){var a=students.find(function(x){return x.ID_ALUNO===$("#attStudent").value});if(a){$("#attName").value=a.NOME_COMPLETO||"";$("#attType").value="Veterano";$("#attSerie").value=a["SÉRIE"]||"";drawCatalog()}};
  $("#clearAttend").onclick=function(){state.currentAttendanceId="";state.attendanceItems=new Set();state.attendanceStage="Contato";renderAtendimento()};
  $("#saveAttendance").onclick=async function(){var f=$("#attForm");if(!f.reportValidity())return;var data=Object.fromEntries(new FormData(f).entries());data.ID_ALUNO=$("#attStudent").value||"";data.ETAPA=state.attendanceStage;data.PROGRESSO=GF_PCT[state.attendanceStage];data.STATUS=state.attendanceStage==="Matriculado"?"Matriculado":state.attendanceStage==="Não converteu"?"Perdido":"Em andamento";var selected=gfCatalog(products,Number(data.ANO_LETIVO),data.SERIE_PRETENDIDA).filter(function(p){return state.attendanceItems.has(p.ID_PRODUTO)}).map(function(p){return {ID_PRODUTO:p.ID_PRODUTO,PRODUTO:p.PRODUTO,CATEGORIA:p.CATEGORIA,SERIE:data.SERIE_PRETENDIDA,QTD:1,VALOR_TABELA:Number(p.VALOR_BASE||0),DESCONTO:0,VALOR_APRESENTADO:Number(p.VALOR_BASE||0),SELECIONADO:"Sim",OBSERVACAO:p["OBSERVAÇÃO"]||""}});try{var r=await api("salvarAtendimento",{token:tokenFor("staff"),data:data,itens:selected});state.currentAttendanceId=r.id;var hid=$("#attForm [name=ID_ATENDIMENTO]");if(hid)hid.value=r.id;setNotice("Atendimento "+esc(r.id)+" salvo. Agora você pode solicitar desconto ou continuar para o fechamento.","ok");$("#saveAttendance").textContent="Atualizar atendimento"}catch(e){alert(e.message)}};
  drawCatalog();
}
